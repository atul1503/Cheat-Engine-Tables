"""
Car Speed Rotator — Independent Momentum & Speed Lock Engine
Run as Administrator.

Controls:
- ARROWS: Normal Driving (Grip) & Car Steering Angle while drifting
- Hold LCTRL: Freeze BOTH Momentum Trajectory and Speed Magnitude
- A / D Keys (While holding LCTRL): Rotate the trajectory path independently
- W / S Keys (While holding LCTRL): Fine-tune/scale your locked speed magnitude
"""
...
GRIP_RATIO = -1.5
TRAJECTORY_ROTATE_SPEED = 0.6  # How fast A/D rotate your slide path
DRIFT_SPEED_SCALE = 1.01  # Speed fine-tuning factor via W/S (1.01 = +1% per frame)
...
VK_DOWN = 0x28
VK_LCTRL = 0xA2
VK_A = 0x41  # Rotate path left
VK_D = 0x44  # Rotate path right
VK_W = 0x57  # Increase locked speed
VK_S = 0x53  # Decrease locked speed
...
# =============================================================================
was_ctrl_down = False
locked_magnitude = 0.0
locked_angle = 0.0


def run_tick(handle, z_addr, current_steer):
    global was_ctrl_down, locked_magnitude, locked_angle
    ...
    # Arrow keys handle steering calculation exclusively
    if left and right:
        target_steer = 0.0
    elif left:
    ...
    if lctrl:
        # Initial press: Completely lock both initial speed magnitude and direction angle
        if not was_ctrl_down:
            locked_magnitude = math.sqrt(xs * xs + zs * zs)
            locked_angle = math.atan2(xs, zs)
            was_ctrl_down = True

        # Independent path rotation via A / D keys
        if key_a:
            locked_angle += math.radians(TRAJECTORY_ROTATE_SPEED)
        if key_d:
            locked_angle -= math.radians(TRAJECTORY_ROTATE_SPEED)

        # Mid-drift speed adjustment via W / S keys
        if key_w:
            locked_magnitude *= DRIFT_SPEED_SCALE
        if key_s:
            locked_magnitude /= DRIFT_SPEED_SCALE

        nx = locked_magnitude * math.sin(locked_angle)
        nz = locked_magnitude * math.cos(locked_angle)

        mode_label = "SLIDE"
    else:
        was_ctrl_down = False

        # Standard grip logic when LCTRL is released
    ...
    status_str = f"[{mode_label}] steer={current_steer:+.2f} speed={locked_magnitude:.2f} angle={math.degrees(locked_angle):.1f}°"
...
def main():
    print(
        f"[*] Controls: ARROWS = Drive/Steer Angle | Hold LCTRL = Lock Speed & Trajectory"
    )
    print(
        f"[*] Drift Modifiers (While holding LCTRL): A/D = Rotate Path | W/S = Tune Speed"
    )
